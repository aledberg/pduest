// Stan model for mortality rates using just one category, with interaction between
// year and age groups
...
parameters {
  ...
  real<lower=-10,upper=10> beta[6];
  // offset
  real<lower=-20,upper=20> alpha[6];
}
...
transformed parameters  {
  ...
  for (i in 1:N) {
    lp[i] = beta[1] + beta[2]*sex_woman[i] + beta[3]*age_old[i] +
      beta[4]*year[i]+beta[5]*year[i]*sex_woman[i]+beta[6]*year[i]*age_old[i];
    // Mean
    mu[i] = exp(lp[i])*pTime[i];
    Nmiss[i] = exp(alpha[1] + alpha[2]*sex_woman[i] + alpha[3]*age_old[i]+
		   alpha[4]*year[i]+alpha[5]*year[i]*sex_woman[i]+
		   +alpha[6]*year[i]*age_old[i]);
...
model {
  ...
  //Nmiss~normal(30000,2000);
  //prev~normal(0.01,0.1);
  y ~ poisson(mu);
